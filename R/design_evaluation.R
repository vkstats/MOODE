...
design_evalation<-function(X1,X2)
{
  matrix=matrix(criteria.values(X1),nrow=1,byrow=T)
  colnames(matrix)<-c("Ds","Ls","DP","LP","df","LoF","compound")
  matrix.mse=matrix(criteria.values.mse(X1, X2),nrow=1,byrow=T)
  colnames(matrix.mse)<-c("Ds","DP","LoFDP","mseD","Ls","LP","LoFLP","mseL")#,"compoundD","compoundL")
  matrix.G=matrix(criteria.values.G(X1, X2),nrow=1,byrow=T)
