...
#' Possible values are: "GL", "GD" for Generalised D- and L-optimality (Goos et al., 2005); 
#' "GDP" and "GLP" for Generalised DP- and LP-optimality; 
...
#' @param Nstarts The number of random starts of the search algorithm.
...
#' @param Biter Number of samples for evaluating the MSE criteria.
...
#' @param primary.model The order of the fitted polynomial model. Alternatively polynomial terms can be directly specified through the 'primary.terms' parameter.
#' Possible values are: "first order" -- for main effects only, "second order" -- for full second order polynomial model
#' @param potential.model The order of the potential/extra polynomial terms. Alternatively can be specified through the 'potential.terms' parameter.
#' Possible values are: "linear interactions", "second order" -- for linear interactions and quadratic terms, "third order" -- for third order terms, including cubic terms.
#' @param primary.terms Vector of the names of the primary terms, e.g. "x12" for $x_1^2$ and "x2x3" for $x2x3$.
#' @param potential.terms Vector of the names of the potential terms, e.g. "x23" for $x_2^3$ and "x22x3" for $x2^2x3$.
...
#' @examples
#' 
#'example1_mood <- mood(K = 5, Klev = 3, Nruns = 40, criterion.choice = "GDP", 
#'kappa.Ds = kappa.DP = kappa.LoF = 1./3, Nstarts = 100, tau2 = 0.1, primary.model = "second order",
#' potential.model = NA, potential.terms = c("x12x2", "x22x3", "x32x4", "x42x5"))
#'example1_mood
#'
#'# Let's change the criterion for the same experiment and models  
#'update_mood = list("criterion.choice" = "MSE.D", "kappa.Ds" = 0, "kappa.mse" = 1./3, Biter = 200)
#'replace(example1_smood, names(update_mood), update_mood) 
#'example1_mood
#'  

mood <- function(K, Klev, Levels, 
                 Nruns = 40, 
  ...
  if (is.na(primary.terms)) {  # if primary terms are not specified explicitly
    primary.terms = c()
    if (any(primary.model %in% c("main effects","first order", "second order",
                                 "third order", "cubic terms"))) {
      for (k in 1:K){
        primary.terms = c(primary.terms, paste("x", as.character(k), sep = ""))
      }
    }
    
    if (any(primary.model %in% c("second order", "third order", "cubic terms"))){
      for (k in 1:K){      # quadratic terms
        primary.terms = c(primary.terms, paste("x", as.character(k), as.character(2), sep = ""))
      }
    }
    
    if (any(primary.model %in% c("first order", "second order", 
                                 "third order", "cubic terms"))){
    ...
    if ("third order" %in% primary.model){
      for (k in 1:(K-1)){
    ...
    if ("cubic terms" %in% primary.model){
  ...
  if (is.na(potential.terms)) {
    ...
    if ("third order" %in% potential.model){
      ...
      } else {print("Warning: no third order terms can be added, too few factors.")}
    ...
    if ("cubic terms" %in% potential.model){
      for (k in 1:K){     # cubic terms: "x13", "x33", etc.
        potential.terms = c(potential.terms, 
                            paste("x", as.character(k) , as.character(3), sep = ""))   
      }
    }
    if ("fourth order" %in% potential.model){
      if (K>1){
        for (k in 1:(K-1)){
          for (j in (k+1):K) { 
      ...
      } else {print("Warning: no fourth order terms can be added, too few factors.")}
  ...
  if ((is.na(potential.terms) || is.null(potential.terms) || (length(potential.terms) == 0)) && 
    ...
    kappa.LoF = kappa.mse = kappa.bias = 0
    print ("No potential terms have been specified. Corresponding weights have been set to 0. 
           Please check the other weights.")
  }
  
  kappa.all = c(kappa.Ds, kappa.Ls, kappa.DP, kappa.LP, kappa.LoF, kappa.bias, kappa.mse)
